...
3. **Facilitator edition is live at /facilitator/** — generated by
   `python3 tools/build-facilitator.py` from index.html + facilitator/notes.json.
   Its QR encodes the LEARNER url so the room scans off the projected screen.
   REGENERATE it after any change to index.html or notes.json, then publish.
